...
void register_boost_devfreq(struct devfreq *devfreq)
{
	tz_devfreq_g = devfreq;
	lowest_freq = devfreq->profile->freq_table[devfreq->profile->max_state - 1];
}
EXPORT_SYMBOL(register_boost_devfreq);
...
int gpu_load_boost_event(struct devfreq_dev_status stats)
{
	...
	if (!stats.total_time || (stats.busy_time * 100 / stats.total_time) < load_threshold)
		return 0;
